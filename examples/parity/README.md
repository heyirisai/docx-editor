...
The 1.0.0 unification renames packages and ships React and Vue adapters from a shared `@heyirisai/docx-editor-core`. The community-trust signal is "the same DOCX renders identically in both adapters when installed from npm." This deployment proves it by serving both adapters from real `node_modules` resolution paths, not source aliases.

The build trick: dev mode aliases `@heyirisai/*` to `packages/*/src` for HMR. Parity mode sets `USE_PUBLISHED_PACKAGES=true` so vite resolves through `node_modules` → workspace `dist/` — the exact bytes a consumer downloads from npm.
...
| `/react/` | `examples/vite/dist` | React (`@heyirisai/docx-editor-react`) |
| `/vue/`   | `examples/vue/dist`  | Vue (`@heyirisai/docx-editor-vue`)     |
