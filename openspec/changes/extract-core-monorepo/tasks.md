...
- [x] 1.3 Create `packages/core/package.json` with name `@heyirisai/docx-editor-core`, dependencies (prosemirror-\*, jszip, pizzip, xml-js, docxtemplater, clsx), and exports config
- [x] 1.4 Create `packages/react/package.json` with name `@heyirisai/docx-editor-react`, dependency on `@heyirisai/docx-editor-core`, peerDependencies (react, react-dom), and `@radix-ui/react-select`
- [x] 1.5 Create `packages/vue/package.json` with name `@heyirisai/docx-editor-vue`, dependency on `@heyirisai/docx-editor-core`, peerDependency on `vue`, and a placeholder `src/index.ts`
...
- [x] 5.3 Export `EditorPluginCore`, `PluginPanelProps`, `PanelConfig`, `RenderedDomContext` from `@heyirisai/docx-editor-core`
...
- [x] 5.8 Export `ReactEditorPlugin` from `@heyirisai/docx-editor-react` and `VueEditorPlugin` from `@heyirisai/docx-editor-vue`

## 6. Update Internal Imports

- [x] 6.1 Update all imports in `packages/react/` that reference core modules to use `@heyirisai/docx-editor-core` instead of relative paths
- [x] 6.2 Keep `packages/react/src/index.ts` scoped to React-owned adapter exports; import core utilities from `@heyirisai/docx-editor-core`
- [x] 6.3 Do not publish a legacy `@heyirisai/docx-js-editor` shim in 1.x; import canonical packages directly
...
- [x] 10.1 Verify `@heyirisai/docx-editor-react` main export includes all previously exported symbols
- [x] 10.2 Verify subpath exports (`/core`, `/headless`, `/react`, `/ui`, `/core-plugins`, `/mcp`) still work from the React package
- [x] 10.3 Verify `@heyirisai/docx-editor-core` can be imported and used without React installed
