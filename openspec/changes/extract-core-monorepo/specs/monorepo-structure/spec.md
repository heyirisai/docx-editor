...
The `packages/core/` directory SHALL contain all framework-agnostic source code with its own `package.json` named `@heyirisai/docx-editor-core`.
...
The `packages/react/` directory SHALL contain all React-dependent source code with its own `package.json` named `@heyirisai/docx-editor-react`.
...
- **THEN** `@heyirisai/docx-editor-core` SHALL be listed in dependencies
...
- **AND** it SHALL externalize `@heyirisai/docx-editor-core` (not bundle it)
...
- **THEN** tests SHALL import from both `@heyirisai/docx-editor-core` and `@heyirisai/docx-editor-react`
...
- **AND** the app SHALL render a functional DOCX editor using `@heyirisai/docx-editor-vue` and `@heyirisai/docx-editor-core`
...
The `packages/vue/` directory SHALL contain an empty scaffolded package named `@heyirisai/docx-editor-vue` with a dependency on `@heyirisai/docx-editor-core`, ready for community contribution.

#### Scenario: Vue package has minimal package.json

- **WHEN** inspecting `packages/vue/package.json`
- **THEN** it SHALL have name `@heyirisai/docx-editor-vue`
- **AND** `@heyirisai/docx-editor-core` SHALL be listed in dependencies
