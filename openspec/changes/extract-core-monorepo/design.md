...
- Extract framework-agnostic core into `@heyirisai/docx-editor-core` with zero React dependencies
- Keep `@heyirisai/docx-editor-react` as the React UI package (preserves npm stats, no migration for existing users)
- Use Bun workspaces for monorepo management (already using Bun as runtime)
- Enable community Vue/Svelte wrappers that depend only on `@heyirisai/docx-editor-core`
...
**Decision:** Split into exactly `@heyirisai/docx-editor-core` and `@heyirisai/docx-editor-react` (React).
...
**Decision:** `@heyirisai/docx-editor-react` stays as the React package name. No rename.
...
**Core (`@heyirisai/docx-editor-core`):**
...
**React (`@heyirisai/docx-editor-react`):**
...
**Decision:** React package imports from `@heyirisai/docx-editor-core` (the npm package name), not relative paths.
...
**Core interface (`@heyirisai/docx-editor-core`):**
...
**React adapter (`@heyirisai/docx-editor-react`):**
...
**Vue adapter (`@heyirisai/docx-editor-vue`):**
...
- `EditorPluginCore`, `PanelConfig`, `PluginPanelProps` (without ReactNode), `RenderedDomContext` → `@heyirisai/docx-editor-core`
...
**[Risk] Internal import churn** → Files that currently do `../docx/parser` will need `@heyirisai/docx-editor-core` imports. Mitigation: batch find-and-replace, verify with typecheck.
