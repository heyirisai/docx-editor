...
- **BREAKING**: Split the single `@heyirisai/docx-editor-react` package into a Bun workspaces monorepo with separate packages
- New `@heyirisai/docx-editor-core` package containing all framework-agnostic code: `src/docx/`, `src/types/`, `src/prosemirror/`, `src/layout-engine/`, `src/layout-painter/`, `src/layout-bridge/`, `src/core-plugins/`, `src/utils/` (non-React parts), `src/agent/`, `src/mcp/`
- `@heyirisai/docx-editor-react` becomes a thin React UI package depending on `@heyirisai/docx-editor-core`, containing: `src/components/`, `src/hooks/`, `src/plugin-api/`, `src/plugins/`, `src/paged-editor/`
- Remove React type leaks from core code paths (e.g., `CSSProperties` imports in utils)
- Split `EditorPlugin` interface into a framework-agnostic core (`EditorPluginCore` in `@heyirisai/docx-editor-core`) and framework-specific adapters (`ReactEditorPlugin` in React package, `VueEditorPlugin` in Vue package)
- Existing `@heyirisai/docx-editor-react` keeps its npm name and download stats — existing users continue using it as before
...
- `core-package-api`: Public API surface for `@heyirisai/docx-editor-core` — exports for DOCX parsing/serialization, ProseMirror schema/extensions/commands, layout engine, document model types, and headless editor creation
...
- **npm**: New `@heyirisai/docx-editor-core` package published; `@heyirisai/docx-editor-react` gains `@heyirisai/docx-editor-core` as dependency; `@heyirisai/docx-editor-vue` scaffolded for community contribution
...
- **Downstream**: Vue/Svelte/vanilla consumers can depend on `@heyirisai/docx-editor-core` directly with zero React overhead
