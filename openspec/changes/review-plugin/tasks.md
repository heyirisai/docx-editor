...
- [x] 1.1 Create `packages/agent-use/` with `package.json` (`@heyirisai/docx-editor-agents`, BSL-1.1, peer dep on `@heyirisai/docx-editor-core`), `tsconfig.json`, `tsup.config.ts` with two entry points (`src/index.ts`, `src/bridge.ts`)
