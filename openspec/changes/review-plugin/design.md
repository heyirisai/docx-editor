...
Monorepo: `packages/core` (`@heyirisai/docx-editor-core`, MIT), `packages/react`, `packages/vue`. Root workspace `"packages/*"`. Build: tsup. New package: `packages/agent-use` → `@heyirisai/docx-editor-agents` (BSL-1.1).
...
`packages/agent-use/` with `@heyirisai/docx-editor-core` as peer dep. Published on public npm under BSL-1.1.
