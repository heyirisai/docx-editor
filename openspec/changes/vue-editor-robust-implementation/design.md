...
The strategic constraint: we keep three packages with hard boundaries — `@heyirisai/docx-editor-core` (no UI framework), `@heyirisai/docx-editor-react` (React-only UI), `@heyirisai/docx-editor-vue` (Vue-only UI). Cross-imports between adapters or from core into a framework break the whole thesis of "core is framework-agnostic."
...
- Re-architecting `@heyirisai/docx-editor-core` for Vue. The core is already framework-agnostic; if Vue can't consume it cleanly, that's a Vue-side adapter problem.
- A unified test runner that's Vue-aware. Playwright drives the browser; framework specifics are invisible at that layer.
- Re-architecting `@heyirisai/docx-editor-agents`. The bridge, server, mcp, ai-sdk subpaths stay framework-agnostic; only the UI subpath gets a Vue twin alongside the existing React one.
...
**Why:** Vue idiom is "components for layouts, composables for logic." Forcing component-only would push power users into manual DOM mounting; forcing composable-only would make the simple "drop in an editor" path verbose. Both paths share one underlying `EditorCore` from `@heyirisai/docx-editor-core`, no duplication.
...
**Why:** the bridge is supposedly "framework-agnostic" but the same module re-exports React hooks. A Vue consumer importing `@heyirisai/docx-editor-agents/bridge` would pull `react` as a transitive dep through the re-export chain. Breaks the framework-isolation contract.
...
**Implication:** import paths shift slightly. Anyone importing `useAgentChat` from `@heyirisai/docx-editor-agents` (the bare entry) breaks; they need `@heyirisai/docx-editor-agents/react`. Document in the change's CHANGELOG; keep the bare-entry export for one minor version with a deprecation notice if back-compat matters.

### Decision 9: AI SDK adapter — Vue needs `@ai-sdk/vue`, not `@ai-sdk/react`

**Choice:** the Vue agent SDK uses `@ai-sdk/vue` (Vercel AI SDK's official Vue bindings) for streaming chat state, mirroring how the React side uses `@ai-sdk/react`. `@ai-sdk/vue` becomes an optional peer dep of `@heyirisai/docx-editor-agents` alongside the existing `ai` peer.
...
### Decision 12: `@heyirisai/docx-editor-agents` description + README rewrite is a precondition
...
**Alternative considered:** ship `@heyirisai/docx-editor-agents-vue` as a separate package. Rejected — doubles the publish surface, doubles the changelog, and creates a versioning mismatch where the Vue agent SDK could lag the React one.
...
**Choice:** keep all five packages (`@heyirisai/docx-editor-react`, `-vue`, `-core`, `-agents`, `-js-editor` shim) in the `.changeset/config.json` `fixed` group through the 1.x line. No public roadmap commitment to a future split.
...
- **Adding a method** to `EditorRefLike` is a **minor** bump in `@heyirisai/docx-editor-agents`, but requires a **coordinated minor bump** in both editor adapters in the same release train so both implement the new method. The fixed group enforces this.
...
| **i18n validation**           | `i18n:validate` + prepublish                     | <1s   | Translation key drift in the shared `@heyirisai/docx-editor-i18n` locale files                                         |
...
- What's the agent SDK story for Vue? React has `@heyirisai/docx-editor-agents/react` with hook-based subscriptions. Vue would need composable equivalents. **Tentative answer:** out of v1 scope; document as a known omission and file a follow-up issue.
