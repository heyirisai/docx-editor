...
_None._ This is a foundation drop on a long-living release branch; existing `@heyirisai/docx-editor-react` and `@heyirisai/docx-editor-core` requirements don't change.
...
- **Public API surface**: `@heyirisai/docx-editor-vue` exports stabilize. The first version that drops `[STUB]` is effectively the package's API contract for the 1.x line; future breaking changes need a major bump.
