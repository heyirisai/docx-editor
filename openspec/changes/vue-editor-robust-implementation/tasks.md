...
- [x] 9.0c Add `@heyirisai/docx-editor-agents` README to root README's package table
...
- [x] 9.5a Drop the legacy `@heyirisai/docx-js-editor` shim from the 1.x train entirely. New surfaces — agent SDK, Vue adapter, core/headless APIs — ship only under their canonical package names.
- [x] 9.5c Done as part of §9.2 — `packages/react/src/index.ts` no longer re-exports agent UI. `@heyirisai/docx-editor-agents/react` is the single canonical import path. Breaking change documented in the 1.0 changeset.
...
- [x] 12.4 Add a Vue section to root `README.md` Quick Start showing both `npm install @heyirisai/docx-editor-react` and `npm install @heyirisai/docx-editor-vue` paths
...
- [ ] 13.8 Remove any `[BETA]` or stub marker from `@heyirisai/docx-editor-agents/vue` description / Vue subpath docs
...
- [ ] 14.6 When the 1.0.0-release train is ready to ship to `main`, Vue and the new `@heyirisai/docx-editor-agents/vue` subpath ride along at 1.0.0 with the fixed group
