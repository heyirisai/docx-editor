...
- Public API: `SdtProperties`/`BlockSdt`/`SdtType`/`InlineSdt` are `@public`; `bun run api:extract` is **mandatory** (the snapshot will change). Changeset: `patch` for `@heyirisai/docx-editor-core`.
