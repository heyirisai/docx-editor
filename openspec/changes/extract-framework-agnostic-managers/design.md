...
A Vue contributor would need to reimplement all of this logic. The goal is to extract the state machines and coordination logic into framework-agnostic classes in `@heyirisai/docx-editor-core`.

## Goals / Non-Goals

**Goals:**

- Extract framework-agnostic state machines and coordination logic from React components into plain TypeScript classes in `@heyirisai/docx-editor-core`
...
- Changing any public API of `@heyirisai/docx-editor-react`
...
### 4. Keep managers in `@heyirisai/docx-editor-core` as plain classes

**Decision:** All manager/coordinator classes live in `packages/core/src/managers/` and are exported from `@heyirisai/docx-editor-core`.
