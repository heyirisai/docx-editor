...
| Piece                                                         | Where                                         | Notes                                                       |
| ------------------------------------------------------------- | --------------------------------------------- | ----------------------------------------------------------- |
| `<DocxEditor agentPanel>`                                     | `@heyirisai/docx-editor-react`                | Right-hand panel, sparkle toolbar toggle, drag-resize       |
| `<AgentChatLog>`, `<AgentComposer>`, `<AgentTimeline>`        | `@heyirisai/docx-editor-agents/react`         | Opinionated chat primitives — optional                      |
| `useDocxAgentTools()`                                         | `@heyirisai/docx-editor-agents/react`         | Tool executor + selection / page context                    |
| `getToolSchemas()`, `getToolDisplayName()`, `executeToolCall` | `@heyirisai/docx-editor-agents/server`        | OpenAI function-calling format — runtime-agnostic           |
| `getAiSdkTools()`                                             | `@heyirisai/docx-editor-agents/ai-sdk/server` | **AI SDK only.** Returns `streamText({ tools })` shape      |
| `toAgentMessages()`                                           | `@heyirisai/docx-editor-agents/ai-sdk/react`  | **AI SDK only.** `useChat` `UIMessage[]` → `AgentMessage[]` |
| `DocxReviewer`                                                | `@heyirisai/docx-editor-agents` (Node)        | Headless: same toolkit, no editor                           |
...
```bash
npm i @heyirisai/docx-editor-react @heyirisai/docx-editor-agents \
...
import { type AgentContextSnapshot } from '@heyirisai/docx-editor-agents/server';
import { getAiSdkTools } from '@heyirisai/docx-editor-agents/ai-sdk/server';
...
import { type DocxEditorRef } from '@heyirisai/docx-editor-react';
import {
  AgentChatLog,
  AgentComposer,
  useDocxAgentTools,
  getToolDisplayName,
  type EditorRefLike,
} from '@heyirisai/docx-editor-agents/react';
import { toAgentMessages } from '@heyirisai/docx-editor-agents/ai-sdk/react';

const DocxEditor = dynamic(
  () => import('@heyirisai/docx-editor-react').then((m) => ({ default: m.DocxEditor })),
  { ssr: false }
);
...
```ts
import { getToolSchemas, executeToolCall } from '@heyirisai/docx-editor-agents/server';
...
import { getToolSchemas } from '@heyirisai/docx-editor-agents/server';
...
```tsx
import type { AgentToolDefinition } from '@heyirisai/docx-editor-agents/react';
