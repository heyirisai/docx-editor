...
- **Drawing shapes**: round-trip. A shape with a fill or an outline and no
  ...
  templates are built from — so text laid over one stays legible. Connectors
  and text inside a shape are not painted yet, and custom geometry both paints
  and saves as its bounding rectangle.
