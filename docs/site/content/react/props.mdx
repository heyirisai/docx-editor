...
| `commentIdNamespace` | `number` | Collab only: per-client unique 32-bit integer (with Yjs, `doc.clientID`). Each client mints comment/tracked-change IDs in a private block, so two peers can never mint the same ID. Omit outside collaboration for the classic small sequential IDs. |
| `historyOverride` | `HistoryOverride` | Collab only: replaces built-in undo/redo on the body editor with caller-supplied commands, so y-prosemirror's `yUndoPlugin` can own history (users undo only their own edits). Latched on mount. |
