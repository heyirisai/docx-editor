 ...
 *
 * The caller measures its own runs (the painter and the layout measurer have
 ...
 */
export interface TabFollowingContent {
 ...
 * the width needed to reach it. For `end`/`center`/`decimal` stops the
 * content after the tab is aligned against the stop, so the caller supplies
 * its measured width via {@link TabFollowingContent}.
 *
