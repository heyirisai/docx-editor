...
<p align="center">
  <a href="https://www.npmjs.com/package/@heyirisai/docx-editor-agents"><img src="https://img.shields.io/npm/v/@heyirisai/docx-editor-agents.svg?style=flat-square&color=3B5BDB" alt="npm version" /></a>
  <a href="https://www.npmjs.com/package/@heyirisai/docx-editor-agents"><img src="https://img.shields.io/npm/dm/@heyirisai/docx-editor-agents.svg?style=flat-square&color=3B5BDB" alt="npm downloads" /></a>
...
```bash
npm install @heyirisai/docx-editor-agents
```

```ts
import { readFile, writeFile } from 'node:fs/promises';
import { DocxReviewer } from '@heyirisai/docx-editor-agents';
...
```ts
// React
import { useAgentChat } from '@heyirisai/docx-editor-agents/react';
const { executeToolCall, toolSchemas } = useAgentChat({ editorRef, author: 'Assistant' });

// Vue
import { useAgentBridge } from '@heyirisai/docx-editor-agents/vue';
const { executeToolCall, toolSchemas } = useAgentBridge({ editorRef, author: 'Assistant' });
```

Both share the same `EditorRefLike` contract from `/bridge`, the same tool catalog, and the same `AgentMessage[]` chat shape. For other frameworks, build the bridge directly via `createEditorBridge` from `@heyirisai/docx-editor-agents/bridge`.
...
| Subpath                                       | Use when                                                       |
| --------------------------------------------- | -------------------------------------------------------------- |
| `@heyirisai/docx-editor-agents`               | Server-side review, library glue                               |
| `@heyirisai/docx-editor-agents/bridge`        | Wiring AI tools into a running editor adapter                  |
| `@heyirisai/docx-editor-agents/server`        | Backend routes needing agent tooling without the MCP transport |
| `@heyirisai/docx-editor-agents/mcp`           | Building an MCP server (any transport)                         |
| `@heyirisai/docx-editor-agents/ai-sdk/server` | Server-side streaming chat with the Vercel `ai` package        |
| `@heyirisai/docx-editor-agents/react`         | React apps wiring `<DocxEditor>` to an agent                   |
| `@heyirisai/docx-editor-agents/ai-sdk/react`  | React chat UI over the bridge                                  |
| `@heyirisai/docx-editor-agents/vue`           | Vue apps wiring `<DocxEditor>` to an agent                     |
| `@heyirisai/docx-editor-agents/ai-sdk/vue`    | Vue chat UI over the bridge                                    |
...
```ts
import type { WordCompatBridge } from '@heyirisai/docx-editor-agents';
```
